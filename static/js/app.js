...
const TIPS = {
  ...
  Hybrid:     ['🏙️ Hybrids shine in stop-and-go city traffic.','🔋 Plug-in hybrids can run purely on electric for typical daily commutes.','🌱 Your LCA emissions are already well below the average petrol car.'],
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Nav buttons: Home / Calculator / About
...
function updateEmInfo() {
  const el = document.getElementById('emInfoBox');
  if (el) el.innerHTML = EM_INFO[state.emMode] || '';
}
...
function setupSearch(inputId, listId, selectedId, onSelect) {
  ...
  if (!inp || !list) return;
  list._onSelect = (id) => { onSelect(id); showSelected(selectedId, listId, inputId, id); };
...
function renderList(listId, query = '') {
  ...
  if (!filtered.length) {
    list.innerHTML = `<div style="padding:.8rem 1rem;color:var(--muted);font-size:.82rem;">No vehicles found</div>`;
  } else {
    filtered.forEach(([id, v]) => {
      ...
      item.innerHTML = `
        ...
        <div style="flex:1"><div class="vi-name">${v.name}</div><div class="vi-type">${v.type}</div></div>
        <div class="vi-right">
          <div class="vi-tp">🏭 ${v.tailpipe}</div>
          <div class="vi-lca">🔄 ${v.lca}</div>
          <div class="vi-disp">♻️ ${v.disposable}</div>
        </div>`;
  ...
  el.innerHTML = `
    ...
    <div class="vs-info">
      ...
      <div class="vs-factors">
        ...
        <span class="dp">Disp: ${v.disposable} kg/km</span>
      </div>
    </div>
...
async function doCalculate() {
  if (!state.selectedId) { alert('Please select a vehicle first.'); return; }
...
async function doCompare() {
  ...
  try {
    ...
    showCompareResults(await res.json());
  } catch(e) { alert('Server error. Is the backend running?'); }
}
...
function showResults(d) {
  ...
  document.getElementById('resMeta').textContent = `${d.vehicle_type} · ${d.emission_mode.toUpperCase()} · ${d.factor_used} kg CO₂/km`;
  const pill = document.getElementById('ratingPill');
  pill.textContent = d.rating;
  pill.style.cssText = `background:${d.rating_color}22;color:${d.rating_color};border:1px solid ${d.rating_color}55;`;
...
function renderBreakdown(containerId, d) {
  ...
  if (!container) return;
  const rows = [
    { label:'🏭 Tailpipe', cls:'tp',   factor: d.tailpipe_factor },
    { label:'🔄 LCA',      cls:'lca',  factor: d.lca_factor },
    { label:'♻️ Disposal', cls:'disp', factor: d.disposable_factor },
  ];
  ...
  container.innerHTML = rows.map(r => {
    ...
    return `<div class="bp-row">
      ...
      <div class="bp-track"><div class="bp-fill ${r.cls}" style="width:0%" data-w="${pct}"></div></div>
      <span class="bp-val">${r.factor} kg/km</span>
      <div class="${r.cls === activeMode ? 'bp-active-dot' : 'bp-inactive-dot'}"></div>
    </div>`;
  }).join('');
  setTimeout(() => {
...
function showCompareResults(data) {
  ...
  document.getElementById('sideBySide').innerHTML =
    sbsBox(v1, aWins ? 'winner' : 'loser') +
    `<div class="sbs-divider">${aWins ? '← GREENER' : 'GREENER →'}</div>` +
    sbsBox(v2, !aWins ? 'winner' : 'loser');
  ...
  document.getElementById('cmpBreakdown').innerHTML = `
    ...
    <div class="cbt-row header"><div class="cbt-label">Type</div><div>${vA ? vA.name : 'A'}</div><div>${vB ? vB.name : 'B'}</div></div>
    ${[{label:'🏭 Tailpipe',k:'tailpipe',cls:'tp'},{label:'🔄 LCA',k:'lca',cls:'lca'},{label:'♻️ Disposal',k:'disposable',cls:'disp'}]
      .map(r=>`<div class="cbt-row"><div class="cbt-label">${r.label}</div><div class="cbt-cell"><span class="dot ${r.cls}"></span>${vA?vA[r.k]:'-'} kg/km</div><div class="cbt-cell"><span class="dot ${r.cls}"></span>${vB?vB[r.k]:'-'} kg/km</div></div>`).join('')}`;
  const maxV = Math.max(v1.annual_co2_kg, v2.annual_co2_kg) || 1;
  document.getElementById('chartSection').innerHTML = `
    <div class="ch-row"><span class="ch-name">${v1.vehicle_name}</span><div class="ch-track"><div class="ch-fill a" id="chA" style="width:0%"></div></div><span class="ch-val">${v1.annual_co2_kg.toLocaleString()} kg</span></div>
    <div class="ch-row"><span class="ch-name">${v2.vehicle_name}</span><div class="ch-track"><div class="ch-fill b" id="chB" style="width:0%"></div></div><span class="ch-val">${v2.annual_co2_kg.toLocaleString()} kg</span></div>`;
  setTimeout(() => {
  ...
  document.getElementById('verdictBox').innerHTML = `🌿 <strong>${greenerName}</strong> emits <strong>${data.diff_kg.toLocaleString()} kg less CO₂/year</strong> — <strong>${pctSaved}% cleaner</strong>. Equivalent to planting <strong>${Math.round(data.diff_kg/21)} trees</strong> every year. 🌳`;
}

function sbsBox(v, cls) {
  return `<div class="sbs-box ${cls}">
      ...
      data.map(d => `
        <div class="hist-item" id="hist-${d._id}">
          ...
          <div class="hist-right">
            <div><div class="hist-co2">${d.annual_co2_kg?.toLocaleString()}</div><div class="hist-unit">kg CO₂/yr</div></div>
...
function animateNum(id, to) {
  ...
  const tick = (now) => {
    ...
    el.textContent = (to * (1 - Math.pow(1 - p, 3))).toLocaleString(undefined, { maximumFractionDigits: 1 });
